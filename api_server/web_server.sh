...
gunicorn -k uvicorn.workers.UvicornWorker -b :$PORT -w 4 \
  --access-logfile=/DVLF/api_server/access.log \
  --error-logfile=/DVLF/api_server/error.log \
  --chdir /DVLF web_app:app
